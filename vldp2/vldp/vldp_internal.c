...
int idle_handler(void *surface)
{
	...
	*/
	...
	//==============
	 s_paused = 0;	// whether the video is to be paused
	 s_step_forward = 0;	// whether to step 1 frame forward
	 s_blanked = 0;	// whether the mpeg video is to be blanked
	 s_frames_to_skip = 0;	// how many frames to skip before rendering the next frame (used for P and B frames seeking)
	 s_frames_to_skip_with_inc = 0;	// // how many frames to skip while increasing the frame number (for multi-speed playback)
	 s_skip_all = 0;	// if we are to skip any frame to be displayed (to avoid seeing frames we shouldn't)

	// How many frames we've skipped when s_skip_all is enabled.
	// (for performance tuning, we want to minimize this number)
	 s_uSkipAllCount = 0;

	 s_timer = 0;	// FPS timer used by the blitting code to run at the right speed
	...
	// multi-speed variables ...
	 s_skip_per_frame = 0;	// how many frames to skip per frame (for playing at 2X for example)
	 s_stall_per_frame = 0;	// how many frames to stall per frame (for playing at 1/2X for example)

	 s_bPreCacheEnabled = VLDP_FALSE;	// whether precaching is currently enabled
	 s_uCurPreCacheIdx = 0;	// if pre-caching is enabled, which index is currently being used

	g_mpeg_data = NULL;	// structure for libmpeg2's state
	memset(g_frame_position, 0, MAX_LDP_FRAMES*sizeof(Uint32));
	g_totalframes = 0;	// total # of frames in the current mpeg
