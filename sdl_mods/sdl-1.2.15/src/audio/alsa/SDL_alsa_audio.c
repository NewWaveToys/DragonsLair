...
static void ALSA_CloseAudio(_THIS)
{
	...
	if(mixbuf&&this->spec.samples>0)memset(mixbuf,0,this->spec.samples);
	mixbuf = NULL;
}
...
extern void update_sound(unsigned short *buf, int frames);
#endif
...
#else
 update_sound(mixbuf, this->spec.samples);
