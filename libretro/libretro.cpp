...
void retro_init(void)
{
	...
	if (environ_cb)
	{
		...
		enum retro_pixel_format n_pixelformat = RETRO_PIXEL_FORMAT_XRGB8888;//RETRO_PIXEL_FORMAT_RGB565;
...
#define GIT_VERSION ""
    ...
    out_systeminfo->library_version		= "1.0.7";//GIT_VERSION;
...
int skipcount = 0;
int skipframes = 0;
int readbuf=0,ismixed=0, readlen=0;
#define MIX_LEN 8192
int send_sound(void *p)
{
	#define  A_frames  2940 //2048
	static int16_t ab_buffer[A_frames]={0};
	//static uint8_t mix_buffer[MIX_LEN]={0};

	//memset(ab_buffer, 0, A_frames);
	audio_callback ( NULL, (uint8_t *)ab_buffer, A_frames );
	
	#if 0
	if(!ismixed)
	{
		audio_callback ( NULL, (uint8_t *)mix_buffer, MIX_LEN );
		ismixed = 1;
	}
	if(ismixed)
	{
	
		if((readbuf+A_frames)>MIX_LEN)
		{
			readlen = MIX_LEN-readbuf;
			//printf("sound %d %d\n",readlen, readbuf);
			if(readlen>0)
				memcpy(ab_buffer, mix_buffer+readbuf,readlen);
			audio_callback ( NULL, (uint8_t *)mix_buffer, MIX_LEN );
			
			memcpy(ab_buffer+(readlen>>1), mix_buffer,A_frames-readlen);
			readbuf = A_frames-readlen;
			//printf("sound %d %d =\n",readlen, readbuf);
		}
		else
		{
		
			memcpy(ab_buffer, mix_buffer+readbuf,A_frames);
			readbuf+=A_frames;
		}
	}
	#endif
...
extern "C"
void update_sound(unsigned short *buf, int frames)
{
//printf("sound %d \n", frames);
	...
	if (audio_batch_cb)
	{
		audio_batch_cb((const int16_t*)buf, frames);
	}
//	printf("sound %d end \n", frames);
}
...
if(game_ispause&&game_isresume)
{
	...
	for (int n_numplayers = 0; n_numplayers < RETRO_MAX_PLAYERS; n_numplayers++)
	{
		...
		for (int n_button_ndx = 0; n_button_ndx < RETRO_MAX_BUTTONS; n_button_ndx++)
		{
			...
			{
			if(n_port==1&&n_button_ndx==3)n_button_ndx = 15;
...
void flush_video(void *buf, int w, int h)
{
	if(video_cb)
	{//printf("flush_video\n");
		//send_sound(NULL);
...
int parameter_read(void)
{
    FILE * fpFile = 0;

    fpFile = fopen(PARAMETER_FILE, "rb+");
    if (fpFile <= 0) {
        printf("parameter read fail\n");
        return -1;
    }
...
bool retro_load_game(const struct retro_game_info *in_game)
{
	...
	strcpy(str_args[num_args], "-prefer_samples");
	pstr_args[num_args] = str_args[num_args];
	num_args++;

	strcpy(str_args[num_args], "-nolog");
	pstr_args[num_args] = str_args[num_args];
	num_args++;

	if(parameter_read()==0){
		printf("screen_blanking=%d\n",para_data.screen_blanking);
		if(para_data.screen_blanking == 1) {
			strcpy(str_args[num_args], "-blank_searches");
        		pstr_args[num_args] = str_args[num_args];
        		num_args++;
		}
	}
