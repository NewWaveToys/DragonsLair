...
int open_pcm()
{
	//close_pcm();
	if(pcm)return 0;
	int ret = snd_pcm_open(&pcm, SND_DEVICE,SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK);
	printf("%s ret %d\n",__FUNCTION__,ret);
	if(ret < 0)
	{close_pcm();	return -1;}
		...
		if(play_index>=0)
		{
			...
			if(play_index == 0)
				...
				play_dingpcm();
					...
					play_wav("/etc/res/music/1khz0dbLR.wav");
			}	
			else if(play_index == 3){
					play_wav("/etc/res/music/2khz0dbR.wav");
					
			}
			else if(play_index == 4){
				
					play_wav("/etc/res/music/1khz0dbL.wav");
				
			}
			else if(play_index == 5)
				play_silence();
...
void setCallback(sound_end callback)
{
