...
ssize_t SNDWAV_WritePcm(SNDPCMContainer_t *sndpcm, size_t wcount, size_t load)
{
	ssize_t r;
	ssize_t result = 0;
...
int SNDWAV_SetParams(SNDPCMContainer_t *sndpcm, WAVContainer_t *wav)
{
	...
	sndpcm->wav_data_size = wav->chunk.length;
	
	//printf("==sndpcm->wav_data_size=%d,sndpcm->chunk_bytes===%d\n",sndpcm->wav_data_size,sndpcm->chunk_bytes);
 
	/* Allocate audio data buffer */
	sndpcm->data_buf = (uint8_t *)malloc((wav->chunk.length/sndpcm->chunk_bytes+1)*sndpcm->chunk_bytes);
	if (!sndpcm->data_buf) {
